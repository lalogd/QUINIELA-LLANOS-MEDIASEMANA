...
function selection(element){ 		//Pinta la casilla y actualiza el texto de la quiniela.
    ...
    if (!element.style.backgroundColor){
        ...
        element.style.backgroundColor = "rgb(247, 20, 20)";
    }
    else{
        if(combinations){
...
function save(){				//Se añade la quiniela actual a la lista 
	if(id < 150){
	    ...
	    if (res.join("\xa0\xa0").includes("_"))
	        alert("Debes llenar todos tus pronosticos");
	    else if (!name){
...
function allowcombination(){
    if (!combinations) 
    {
        ...
        document.getElementById("checkcombinaciones").style.backgroundColor = "rgb(247,20,20)";
...
function random(){
    ...
    for (var i = 0; i < nPartidos; i++){
        ...
        partidos[i].getElementsByTagName("span")[r].style.backgroundColor = "rgb(247, 20, 20)";
        res[i] = ["L","E","V"][r];
    }
    container.innerHTML = res.join("\xa0\xa0");
    costoactual();
...
function send(){				//Envia la quiniela al whatsapp 
    ...
    const quinbara = "*QuinielaMS*%0A%0A";
